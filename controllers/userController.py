...
def profile_photo():
    ...
    upload_folder = os.path.join(current_app.static_folder, 'uploads', 'perfiles')
    result = UserService.update_photo(user_id, photo_file, upload_folder)
    if result["success"]:
        flash(result["message"], 'success')
    else:
        flash(result["message"], 'danger')
...
@user_bp.route('/api/clients', methods=['GET'])
@login_required
def api_clients():
    from flask import jsonify
    result = UserService.get_clients()
    return jsonify(result.get("clients", []))